...
import {
  ...
  Modal, message, Divider, List, Badge, Progress, Alert
} from 'antd';
import {
  ClockCircleOutlined, CalendarOutlined, FileSearchOutlined,
  AuditOutlined, ScheduleOutlined, ThunderboltOutlined,
...
const SCHEDULE_DATA: ScheduleShift[] = [
  ...
  { date: '2026-06-16', name: '赵思源', shift: '早班', hours: 8 },
  { date: '2026-06-16', name: '刘欣然', shift: '中班', hours: 8 },
  { date: '2026-06-16', name: '黄一萧', shift: '早班', hours: 8 },
  { date: '2026-06-16', name: '周晓彤', shift: '早班', hours: 8 },
  { date: '2026-06-16', name: '杨振华', shift: '弹性', hours: 8.5 },
];
...
/* ================= Tab 1：考勤报表/统计 ================= */
const ReportTab: React.FC<{ onOpenDetail: () => void }> = ({ onOpenDetail }) => (
  <Row gutter={16}>
    <Col span={16}>
      <Card
      ...
      >
        <Form layout="inline" style={{ marginBottom: 16 }}>
          ...
          <Form.Item>
            <Button type="primary" icon={<FileSearchOutlined />} onClick={onOpenDetail}>查看汇总</Button>
          </Form.Item>
        </Form>
        <Row gutter={16}>
    ...
    <Col span={8}>
      <Card className="section-card" title={<Space><ScheduleOutlined />本月排班速览</Space>}>
        <Table<ScheduleShift>
          size="small"
...
/* ================= Tab 3：假期余额管理 ================= */
const LeaveTab: React.FC<{ onAdjust: (item: LeaveBalance) => void }> = ({ onAdjust }) => (
  <Card
    className="section-card"
    title={<Space><CalendarOutlined />假期余额管理</Space>}
    extra={
      <Space>
        <Select placeholder="员工姓名" style={{ width: 140 }} allowClear options={[
          { label: '程璐', value: '程璐' },
          { label: '王嘉怡', value: '王嘉怡' },
          { label: '张子豪', value: '张子豪' },
        ]} />
  ...
  >
    <Table<LeaveBalance>
      ...
      pagination={false}
      columns={[
        { title: '假期类型', dataIndex: 'type', width: 100 },
        { title: '总额(天)', dataIndex: 'total', width: 90 },
        { title: '已用(天)', dataIndex: 'used', width: 90 },
        { title: '剩余(天)', dataIndex: 'remaining', width: 100, render: (v: number) => <Text strong style={{ color: v > 0 ? '#52c41a' : '#999' }}>{v}</Text> },
        { title: '到期日', dataIndex: 'expiry', width: 110, render: (v?: string) => v || '-' },
        { title: '操作', width: 100, render: (_: any, r: LeaveBalance) => (
  ...
  >
    <Table<ApprovalItem>
      ...
      columns={[
        ...
        { title: '部门', dataIndex: 'department', width: 80, ellipsis: true },
        { title: '时段', render: (_: any, r: ApprovalItem) => <span style={{ fontSize: 12 }}>{r.startTime}<br/>至 {r.endTime}</span>, width: 200 },
...
const ScheduleTab: React.FC = () => (
  <Row gutter={16}>
    <Col span={16}>
      <Card
        className="section-card"
        title={<Space><ScheduleOutlined />排班结果核对（2026-06-16）</Space>}
        extra={
          <Space>
            <DatePicker picker="month" defaultValue={dayjs('2026-06')} format="YYYY-MM" />
      ...
      >
        <Table<ScheduleShift>
          ...
          columns={[
            ...
            { title: '班次', dataIndex: 'shift', width: 80, render: (s: string) => <Tag color={SHIFT_COLOR[s]}>{s}</Tag> },
            { title: '工时', dataIndex: 'hours', width: 80, render: (h: number) => h ? `${h}h` : '-' },
          ]}
        />
      </Card>
    </Col>
    <Col span={8}>
      <Card className="section-card" title={<Space><ThunderboltOutlined />排班规则说明</Space>}>
        <List
          size="small"
          dataSource={[
            { t: '早班', d: '08:30 - 17:30（午休 1h）' },
            { t: '中班', d: '13:00 - 22:00（晚班补贴）' },
            { t: '晚班', d: '17:00 - 次日 02:00' },
            { t: '弹性', d: '8h 弹性工时（8:00-20:00 区间）' },
            { t: '休息', d: '-（按月均周休 2 天）' },
          ]}
          renderItem={(it) => (
            <List.Item>
              <Tag color={SHIFT_COLOR[it.t]} style={{ marginRight: 8 }}>{it.t}</Tag>
              <Text type="secondary" style={{ fontSize: 12 }}>{it.d}</Text>
            </List.Item>
          )}
        />
        <Text type="secondary" style={{ fontSize: 12, display: 'block', marginTop: 8 }}>
          排班表为只读视图；如需调整请去钉钉后台操作。
        </Text>
      </Card>
    </Col>
  </Row>
...
const ShortcutTab: React.FC = () => (
  ...
  >
    <Row gutter={16}>
      {[
        { label: '考勤组设置', desc: '配置规则、定位、WiFi、班次算法', icon: <UserOutlined />, color: '#1890ff' },
...
/* ================= 员工侧 Tab：我的考勤 ================= */
const EmployeeClockTab: React.FC = () => (
  <Row gutter={16}>
    <Col span={8}>
      <Card className="section-card" title={<Space><ClockCircleOutlined />今日打卡</Space>}>
        <Statistic title="上班打卡" value="08:52" prefix={<FieldTimeOutlined />} valueStyle={{ color: '#52c41a' }} />
        <Divider style={{ margin: '12px 0' }} />
        <Statistic title="下班打卡" value="18:05" prefix={<FieldTimeOutlined />} valueStyle={{ color: '#1890ff' }} />
        <Divider style={{ margin: '12px 0' }} />
    ...
    <Col span={8}>
      <Card className="section-card" title={<Space><AuditOutlined />我的审批</Space>}>
        <Row gutter={16}>
          ...
          <Col span={12}><Statistic title="我发起的" value={1} valueStyle={{ color: '#1890ff' }} /></Col>
        </Row>
        <Divider style={{ margin: '12px 0' }} />
        <List
          size="small"
...
/* ================= 员工侧 Tab：我的申请 ================= */
const EmployeeApplyTab: React.FC = () => (
  <Card
    className="section-card"
...
const DailyPage: React.FC = () => {
  ...
  const [selectedLeave, setSelectedLeave] = useState<LeaveBalance | null>(null);
  ...
  const handleSubmitAdjust = async () => {
    try {
      ...
      message.success(`已为【${selectedLeave?.type}】提交调整：${v.delta}天（${v.reason}）`);
    ...
    <div>
      ...
      <div className="stats-row">
        <Card className="stat-card">
          <Statistic title={userIsAdmin ? '今日出勤' : '今日工时'}
            value={userIsAdmin ? 211 : '9.2'}
            suffix={userIsAdmin ? '人' : 'h'}
            prefix={userIsAdmin ? <CheckCircleOutlined /> : <FieldTimeOutlined />}
            valueStyle={{ color: userIsAdmin ? '#52c41a' : '#1890ff' }} />
          <div className="stat-trend">
            <RiseOutlined /> 较昨日 <Text type="success">+3</Text>
          </div>
        </Card>
        <Card className="stat-card">
          <Statistic title={userIsAdmin ? '待审批请假' : '待我审批'}
            value={userIsAdmin ? 7 : 0}
        ...
        <Card className="stat-card">
          <Statistic title={userIsAdmin ? '本月请假' : '我的年假剩余'}
            value={userIsAdmin ? 23 : 7.5}
        ...
        <Card className="stat-card">
          <Statistic title={userIsAdmin ? '本月加班' : '本月迟到'}
            value={userIsAdmin ? 48 : 1}
            suffix={userIsAdmin ? '人次' : '次'}
            prefix={userIsAdmin ? <RiseOutlined /> : <ExclamationCircleOutlined />}
            valueStyle={{ color: userIsAdmin ? '#722ed1' : '#faad14' }} />
          <div className="stat-trend">
      ...
      <Tabs
        ...
        items={userIsAdmin
          ? [
              { key: 'report',   label: <span><FileSearchOutlined />考勤报表/统计</span>, children: <ReportTab onOpenDetail={() => setReportDrawerOpen(true)} /> },
              { key: 'clock',    label: <span><ClockCircleOutlined />原始打卡记录</span>,   children: <ClockTab /> },
              { key: 'leave',    label: <span><CalendarOutlined />假期余额管理</span>,       children: <LeaveTab onAdjust={handleOpenAdjust} /> },
              { key: 'approval', label: <span><AuditOutlined />审批数据备查</span>,          children: <ApprovalTab /> },
              { key: 'schedule', label: <span><ScheduleOutlined />排班结果核对</span>,       children: <ScheduleTab /> },
              { key: 'shortcut', label: <span><ThunderboltOutlined />快捷入口</span>,        children: <ShortcutTab /> },
            ]
          : [
              { key: 'mine',  label: <span><ClockCircleOutlined />我的考勤</span>, children: <EmployeeClockTab /> },
              { key: 'apply', label: <span><CalendarOutlined />我的申请</span>,    children: <EmployeeApplyTab /> },
            ]}
      />
      ...
      {/* 假期余额调整 Modal */}
      <Modal
        title={`假期余额调整 - ${selectedLeave?.type || ''}`}
